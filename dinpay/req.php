 ...
 **/
 ////////////////////////////////////请求参数//////////////////////////////////////
		...
		//后台通知地址(必填)
		$notify_url = "http://".$_SERVER["HTTP_HOST"]."/dinpay/dinpay_callback1.php";

		//定单金额（必填）
		$order_amount = $_POST['price'];

		//商家定单号(必填)
		$order_no = rand(1000,9999).time().rand(1000,9999);
		...
		//页面跳转同步通知地址(选填)
		$return_url = "http://".$_SERVER["HTTP_HOST"]."/dinpay/dinpay_callback.php";
		...
		//公用业务回传参数（选填）
		$extra_return_param =$_POST['username'];
	...
	//if ($merchant_code<>"2010010103"){  echo("<script>location.href='/';</script>");}
	if($return_url != "") {
	...
	**
	...
	if($show_url != "") {
        ...
        //设置密钥
	//$key = "123456789a123456789_"; // <支付密钥> 注:此处密钥必须与商家后台里的密钥一致
	...
	//echo "sign=".$sign."<br>";
					...
					$sql3="INSERT INTO  ssc_savelist set dan='".$order_no."',username='{$extra_return_param}',uid={$userid},bank='dinpay',bankid='0',money=".$order_amount.",rmoney=0,sxmoney=0,adddate='".$datetime."',zt=0, types='2',xf=0";	

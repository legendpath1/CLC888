	...
	if(otype=="input"){
		...
		if(otype=="digital"){
			...
			$.each(opts.layout,function(i,n){
				...
				max_place=n.place>max_place?n.place:max_place;
				data_sel[n.place]=[];
				html+='<div class="nbs">';if(n.cols>0){html+="<div class=ti><div class=l></div>";if(n.title.length>0){html+=n.title}html+="<div class=r></div></div>"}html+='<div class="bl">';numbers=n.no.split("|");temphtml="";if(n.prize){tmpprize=n.prize.split(",")}j=numbers.length;for(i=0;i<j;i++){html+='<div name="lt_place_'+n.place+'">'+numbers[i]+"</div>";if(n.prize){temphtml+="<span>"+$.lt_method_data.prize[parseInt(tmpprize[i],10)]+"</span>"}}html+=temphtml+"</div>"})}}}html+='<div class="c"></div>';
	if(opts.isDm){$.lt_dm=true;}else{$.lt_dm=false}
	if(opts.isDanTuo){$.lt_dantuo=true;$.lt_danlen=parseInt(methodname.substring(methodname.length-1),10)}else{$.lt_dantuo=false}
	$html=$(html);$(this).empty();$html.appendTo(this);$($.lt_id_data.id_desc).html($.lt_method_data.methoddesc);
		...
		function checkNum(){
			...
			if(otype=="input"){
				if(data_sel[0].length>0){
					switch(mname){
						case"LTRZX3":nums=_inputCheck_Num(8,false,_SDinputCheck,false);if(isrx==1){nums*=$.lt_position_sel.length==0?0:Combination($.lt_position_sel.length,3)}break;
						case"LTZU3":nums=_inputCheck_Num(8,false,_SDinputCheck,true);if(isrx==1){nums*=$.lt_position_sel.length==0?0:Combination($.lt_position_sel.length,3)}break;
						case"LTRZX2":nums=_inputCheck_Num(5,false,_SDinputCheck,false);if(isrx==1){nums*=$.lt_position_sel.length==0?0:Combination($.lt_position_sel.length,2)}break;
						case"LTZU2":nums=_inputCheck_Num(5,false,_SDinputCheck,true);if(isrx==1){nums*=$.lt_position_sel.length==0?0:Combination($.lt_position_sel.length,2)}break;
						case"LTRX1":nums=_inputCheck_Num(2,false,_SDinputCheck,false);break;
						case"LTRX2":nums=_inputCheck_Num(5,false,_SDinputCheck,true);break;
						case"LTRX3":nums=_inputCheck_Num(8,false,_SDinputCheck,true);break;
						case"LTRX4":nums=_inputCheck_Num(11,false,_SDinputCheck,true);break;
						case"LTRX5":nums=_inputCheck_Num(14,false,_SDinputCheck,true);break;
						case"LTRX6":nums=_inputCheck_Num(17,false,_SDinputCheck,true);break;
						case"LTRX7":nums=_inputCheck_Num(20,false,_SDinputCheck,true);break;
						case"LTRX8":nums=_inputCheck_Num(23,false,_SDinputCheck,true);break;
						default:break
					}
				}
			}else{
				...
				switch(mname){
					...
					case"LTRZX3":
						nums=0;
						for(ia=0;ia<3;ia++){
							if(data_sel[ia].length>0){
								for(ib=ia+1;ib<4;ib++){
									if(data_sel[ib].length>0){
										for(ic=ib+1;ic<5;ic++){
											if(data_sel[ic].length>0){
												for(i=0;i<data_sel[ia].length;i++){
													for(j=0;j<data_sel[ib].length;j++){
														for(k=0;k<data_sel[ic].length;k++){
															if(data_sel[ia][i]!=data_sel[ib][j]&&data_sel[ia][i]!=data_sel[ic][k]&&data_sel[ib][j]!=data_sel[ic][k]){
																nums++
															}
														}
													}
							...
							if(data_sel[ia].length>0){
								for(ib=ia+1;ib<5;ib++){
									if(data_sel[ib].length>0){
										for(i=0;i<data_sel[ia].length;i++){
											for(j=0;j<data_sel[ib].length;j++){
												if(data_sel[ia][i]!=data_sel[ib][j]){
													nums++
												}
											}
										}
					...
					case"LTDWD":
					case"LTDDS":
						for(i=0;i<=max_place;i++){nums+=data_sel[i].length}break;
					case"LTZU3":
					case"LTZU2":
					case"LTBDW":
					case"LTCZW":
					...
					case"LTRXDT8":
						...
						if(danlen<1||tuolen<1||danlen>=sellen){nums=0}else{nums=Combination(tuolen,sellen-danlen)}
						if(isrx==1){
							if(mname=="LTDTZU3"){
								nums*=$.lt_position_sel.length==0?0:Combination($.lt_position_sel.length,3)
							}else if(mname=="LTDTZU2"){
								nums*=$.lt_position_sel.length==0?0:Combination($.lt_position_sel.length,2)
							}
						}
						break;
					case"LTDM5":
							...
							for(ia=1;ia<12;ia++){
								for(ib=1;ib<12;ib++){
									for(ic=1;ic<12;ic++){
										for(id=1;id<12;id++){
											for(ie=1;ie<12;ie++){
												if(ia!=ib && ia!=ic && ia!=id && ia!=ie && ib!=ic && ib!=id && ib!=ie && ic!=id && ic!=ie && id!=ie){
													numa=0;
													numb=0;
													for(i=0;i<data_sel[0].length;i++){
						...
						if(isrx==1){
							nums*=$.lt_position_sel.length==0?0:Combination($.lt_position_sel.length,5)
						}
						break;
					case"LTDM4":
						nums=0;
			...
			if(isNaN(times)){times=1;$($.lt_id_data.id_sel_times).val(1)}
			...
			if($.lt_dantuo){
				if(data_sel[0].length+1>=$.lt_danlen&&place==0){
					...
					$.each($('div[name="lt_place_0"][class="on"]'),function(i,n){
						if($(this).html()==lastnumber){
							$(this).attr("class","");
							data_sel[0]=$.grep(data_sel[0],function(n,i){return n==lastnumber},true)
						}
					})
				}
				var unplace=1-place;
				if(data_sel[unplace].contains(number)){
					$.each($('div[name="lt_place_'+unplace+'"][class="on"]'),function(i,n){
						if($(this).html()==number){
							$(this).attr("class","");
							data_sel[unplace]=$.grep(data_sel[unplace],function(n,i){return n==number},true)
						}
					})
				}
			}
		...
		$($.lt_id_data.id_sel_insert).unbind("click").click(function(){
			...
			if(otype=="input"){
				...
				switch(mname){
					case"LTRZX3":error=_inputCheck_Num(8,true,_SDinputCheck,false);break;
					case"LTZU3":error=_inputCheck_Num(8,true,_SDinputCheck,true);break;
					case"LTRZX2":error=_inputCheck_Num(5,true,_SDinputCheck,false);break;
					case"LTZU2":error=_inputCheck_Num(5,true,_SDinputCheck,true);break;
					case"LTRX1":error=_inputCheck_Num(2,true,_SDinputCheck,false);break;
					case"LTRX2":error=_inputCheck_Num(5,true,_SDinputCheck,true);break;
					case"LTRX3":error=_inputCheck_Num(8,true,_SDinputCheck,true);break;
					case"LTRX4":error=_inputCheck_Num(11,true,_SDinputCheck,true);break;
					case"LTRX5":error=_inputCheck_Num(14,true,_SDinputCheck,true);break;
					case"LTRX6":error=_inputCheck_Num(17,true,_SDinputCheck,true);break;
					case"LTRX7":error=_inputCheck_Num(20,true,_SDinputCheck,true);break;
					case"LTRX8":error=_inputCheck_Num(23,true,_SDinputCheck,true);break;
					default:break
				}
				if(error.length>0){
			...
			var cfhtml='<tr style="cursor:pointer;"><td class="tl_li_l" width="4"></td><td>'+noshtml.substring(0,20)+"</td><td width=25>"+$.lt_method_data.modes[modes].name+'</td><td width=80 class="r">'+nums+lot_lang.dec_s1+'</td><td width=80 class="r">'+times+lot_lang.dec_s2+'</td><td width=120 class="r">'+money+lot_lang.dec_s3+'</td><td class="c tl_li_r" width="16"><input type="hidden" name="lt_project[]" value="'+serverdata+'" /><input type="hidden" name="lt_jm[]" value="'+$.md5(serverdata+'test009')+'" /></td></tr>';var $cfhtml=$(cfhtml);if($.lt_total_nums==0){$($.lt_id_data.id_cf_content).children().empty()}$cfhtml.prependTo($.lt_id_data.id_cf_content);$('td[class="tl_li_l"]',$cfhtml).parent().mouseover(function(){var $h=$('<div class=fbox><table border=0 cellspacing=0 cellpadding=0><tr class=t><td class=tl></td><td class=tm></td><td class=tr></td></tr><tr class=mm><td class=ml><img src="'+pri_imgserver+'/images/comm/t.gif"></td><td>'+lot_lang.dec_s30+": "+current_methodtitle+"_"+current_methodname+"<br/>"+lot_lang.dec_s31+": "+nohtml+"<br/>"+lot_lang.dec_s32+": "+$.lt_method_data.modes[modes].name+lot_lang.dec_s32+(sel_isdy?(", "+lot_lang.dec_s33+" "+sel_prize+", "+lot_lang.dec_s34+" "+(Math.ceil(sel_point*1000)/10)+"%"):"")+"<br/><div class=in><span class=ic></span>  "+lot_lang.dec_s35+" "+nums+" "+lot_lang.dec_s1+", "+times+" "+lot_lang.dec_s2+", "+lot_lang.dec_s36+" "+money+" "+lot_lang.dec_s3+'</div></td><td class=mr><img src="'+pri_imgserver+'/images/comm/t.gif"></td></tr><tr class=b><td class=bl></td><td class=bm><img src="'+pri_imgserver+'/images/comm/t.gif"></td><td class=br></td></tr></table><div class=ar><div class=ic></div></div></div>');var offset=$(this).offset();var left=offset.left+200;var top=offset.top-79;$(this).openFloat($h,"more",left,top)}).mouseout(function(){$(this).closeFloat()}).click(function(){

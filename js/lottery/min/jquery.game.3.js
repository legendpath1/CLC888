(function($) {
	...
	$.playInit = function(opts) {
		var ps = {
			...
			data_id : {
				...
				id_possibleprize : "#lt_possibleprize",
				id_random_sel_button : "#random_sel_button"
			},
			cur_issue : {
				issue : "20100210-001",
		...
		$.each($.lt_issues, function(i, n) {
			...
			if (j < $.lt_issuecount && endtime >= currentendtime) {
				...
				chtml += '<option value="'
						...
						+ '">'
						+ n.issue
						+ (n.issue == opts.cur_issue.issue ? lot_lang.dec_s7
								: "") + "</option>"
			}
		});
				...
				.live(
						...
						function() {
							...
							var times = parseInt($($.lt_id_data.id_sel_times)
									.val(), 10);
							var modes = parseInt($("#lt_sel_modes").val(), 10);
							var money = Math
									.round(times
											* nums
											* 2
							...
							var sellen = methodname
									.substring(methodname.length - 1);
							...
							for ( var i = 1; i <= prizelevels; i++) {
								html += "<td>"
										+ (isNaN($.lt_method_data.prize[i]) ? "&nbsp;"
												: $.lt_method_data.prize[i])
										+ "</td>"
							}
							...
							for ( var i = ns; i >= min_n[n]; i--) {
								...
								for ( var j = 1; j <= prizelevels; j++) {
									tickets += (ns == 8 && j == 5 && i == 4) ? 0
											: GetCombinCount(i, n + 1 - j)
													* GetCombinCount(ns - i, n
															- (n + 1 - j));
									prizes += (ns == 8 && j == 5 && i == 4) ? 0
											: GetCombinCount(i, n + 1 - j)
													* GetCombinCount(ns - i, n
															- (n + 1 - j))
													* $.lt_method_data.prize[j];
									tmpcodecount = GetCombinCount(i, n + 1 - j)
											* GetCombinCount(ns - i, n
													- (n + 1 - j)) == 0 ? "&nbsp;"
											: GetCombinCount(i, n + 1 - j)
													* GetCombinCount(ns - i, n
							...
							if (n == 7) {
								...
								html += "<tr><td>\u4e2d0\u4e2a\u53f7\u7801</td>";
								for ( var j = 1; j <= prizelevels; j++) {
									tickets += (j == prizelevels ? (ns == 7 ? 1
											: 8) : 0);
									prizes += (j == prizelevels ? (ns == 7 ? 1
											: 8) : 0)
											* $.lt_method_data.prize[5];
									html += "<td>"
											+ (j == prizelevels ? (ns == 7 ? 1
													: 8) : "&nbsp;") + "</td>"
								}
								if (nums - tickets == 0) {
									html += "<td>&nbsp;</td><td>"
							...
							$
									.blockUI( {
										message : '<div style="width:200px;padding:10px 100px;background-color:#fff;border:4px #666 solid;"><img src="'
												+ pri_imgserver
												+ '/images/comm/loading.gif" style="margin-right:10px;">\u6b63\u5728\u8bfb\u53d6\u6295\u6ce8\u8be6\u60c5...</div>',
										overlayCSS : {
											...
											try {
												...
												if (data.stats == "error") {
													$
															.alert('<IMG src="'
																	+ pri_imgserver
																	+ '/images/comm/t.gif" class=icons_mb5_e style="margin:5px 15px 0 0;">'
																	+ data.data)
												} else {
													data = data.data;
													stat = "\u672a\u5f00\u5956";
													...
													html += "<tr><td>\u6ce8\u5355\u7f16\u53f7\uff1a<span>"
															...
															+ (data.project.taskid != 0 ? '&nbsp;<a href="history_taskinfo.php?id='
																	+ data.project.taskid
																							...
																							try {
																								...
																								if (data.stats == "error") {
																									...
																									$
																											.alert('<IMG src="'
																													+ pri_imgserver
																													+ '/images/comm/t.gif" class=icons_mb5_s style="margin:5px 15px 0 0;">\u64a4\u5355\u6210\u529f');
																									$.fn
																											.fastData()
																								}
																							} catch (e) {
																								$
																										.alert('<IMG src="'
																												+ pri_imgserver
																												+ '/images/comm/t.gif" class=icons_mb5_e style="margin:5px 15px 0 0;">\u64a4\u5355\u5931\u8d25\uff0c\u8bf7\u68a2\u540e\u91cd\u8bd5')
																							}
																						}
																					})
																		}
												...
												$
														.alert('<IMG src="'
																+ pri_imgserver
																+ '/images/comm/t.gif" class=icons_mb5_e style="margin:5px 15px 0 0;">\u8bfb\u53d6\u6570\u636e\u51fa\u9519\uff0c\u8bf7\u91cd\u8bd5')
											}
										}
									})
						})
	...
	$.fn.lt_selectarea = function(opts) {
		...
		if (otype == "input") {
			...
			html += '<div class=nbs><table class=ha><tr><td valign=top><textarea id="lt_write_box" style="width:600px;height:80px;"></textarea><br />'
					+ tempdes
								...
								function(k, n) {
									if (typeof (n) == "object") {
										...
										if (typeof (n.minchosen) !== "undefined") {
											minchosen[n.place] = n.minchosen
										}
										html += '<div class="nbs">';
										if (n.cols > 1 && n.title.length > 0) {
											html += "<div class=ti>" + n.title
													+ "</div>"
										} else {
		...
		var _SortNum = function(a, b) {
			if (otype != "input") {
				a = a.replace(/\u5927\.\u5355/g, 0).replace(/\u5927\.\u53cc/g,
						1).replace(/\u5c0f\.\u5355/g, 2).replace(
						/\u5c0f\.\u53cc/g, 3).replace(/\u5927/g, 0).replace(
						/\u5c0f/g, 1).replace(/\u5355/g, 0).replace(/\u53cc/g,
						1).replace(/\u5947/g, 0).replace(/\u5076/g, 1).replace(
						/\u4e0a/g, 0).replace(/\u4e0b/g, 1).replace(/\u4e2d/g,
						2).replace(/\u548c/g, 2).replace(/\s/g, "");
				b = b.replace(/\u5927\.\u5355/g, 0).replace(/\u5927\.\u53cc/g,
						1).replace(/\u5c0f\.\u5355/g, 2).replace(
						/\u5c0f\.\u53cc/g, 3).replace(/\u5927/g, 0).replace(
						/\u5c0f/g, 1).replace(/\u5355/g, 0).replace(/\u53cc/g,
						1).replace(/\u5947/g, 0).replace(/\u5076/g, 1).replace(
									...
									.click(
											function() {
												var sss = '<h4 style="text-align:left;">'
														...
														+ "</h4>";
												if (otype == "digital") {
													sss += '<div class="data" style="height:150px;"><table border=0 cellspacing=0 cellpadding=0><tr><td>'
															+ nos
															+ "</td></tr>";
													var mname = $.lt_method[mid];
													var iSelectNum = parseInt(mname
															.charAt(mname.length - 1));
													var combinationDetail = showCombination(
															nos, iSelectNum);
	...
	$.fn.lt_trace = function() {
		...
		var htmltext = '<span id="lt_margin_html">'
				...
				+ lot_lang.dec_s29
				+ '&nbsp;<input name="lt_trace_margin" type="text" id="lt_trace_margin" value="50" size="3" />'
				+ lot_lang.dec_s3 + '&nbsp;&nbsp;</span>';
				...
				.ajax( {
					...
					success : function(data) {
						...
						if (data.money != "empty") {
							...
							var original = parseFloat($usermoney.val().replace(
									/,/g, ""));
								...
								$.each(diff, function(i, n) {
									if (n == ".") {
										...
										if (n == ",") {
											...
											shtml += '<div class="nub nub' + n
													+ '"></div>'
										}
									}
								});
								if (oridiff > 0) {
									shtml = '<div class="nub nubplus"></div>'
											+ shtml;
									$add.html(shtml)
								} else {
									shtml = '<div class="nub nubreduce"></div>'
											+ shtml;
									$add.html(shtml)
								}
								$add.css("top", "10px");
						...
						$
								.each(
										...
										function(i, n) {
											...
											$shtml += '<td><a href="./history_playinfos.php?id='
													+ n.projectid
													+ '" target="_blank" title="\u67e5\u770b\u6295\u6ce8\u8be6\u60c5" class="blue">'
													+ n.projectid + "</a></td>";

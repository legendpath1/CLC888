...
$_SESSION["mainframe"] = '"./play_jskl3.php"';
...
$sqls="select * from ssc_nums where cid='13' and DATE_FORMAT(endtime, '%H:%i:%s')>='".date("H:i:s")."' order by id asc limit 1";
$rss=mysql_query($sqls) or  die("数据库修改出错1");
$nums=mysql_num_rows($rss);
$dymd=date("ymd");
$dymd2=date("Y-m-d");
if($nums==0){
	$sqls="select * from ssc_nums where cid='13' order by id asc limit 1";
...
if($flag=="gettime"){
	...
	$sqls="select * from ssc_bills where lotteryid='13' and username='".$_SESSION["username"]."' and adddate >='".$htime."' order by id desc limit 10";
	$rss=mysql_query($sqls) or  die("数据库修改出错1");
	$nums=mysql_num_rows($rss);
	echo '[';
	$i=1;
	...
	$sqla="select * from ssc_data where cid='13' and issue='".$_REQUEST['issue']."'";
	...
	if(empty($rowa)){
		...
		echo "{\"code\":[\"".$cn1."\",\"".$cn2."\",\"".$cn3."\",\"".$cn4."\",\"".$cn5."\"],\"issue\":\"".$_REQUEST['issue']."\",\"statuscode\":\"2\"}";//empty
	}
}else if($flag=="read"){
	...
	$sqlc="select * from ssc_data where cid='13' order by issue desc limit 1";
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script language="javascript" type="text/javascript" src="./js/lottery/min/face.13.js?modidate=20130415002"></script>
        <script language="javascript" type="text/javascript" src="./js/lottery/min/methods.13.js?modidate=20130415002"></script>
        <script language="javascript" type="text/javascript" src="./js/lottery/min/jquery.game.4.js?modidate=20130415002"></script>
                ...
                <form>
                    ...
                    <div class="gm_con">
                        <div class="gm_con_tonew">
                            <div class="gct_l">
                                <h3>江苏快三</h3>
                                <p class="gct_now">正在销售 <strong>第 <span id="current_issue"><?=$issue?></span> 期</strong> 总共: <strong><span id="current_sale">82</span></strong> 期</span></p>
                                <div class="clear"></div>
                                <div class="gct_time">
                                ...
                                <div class="gct_menu">
                                    <a class="gct_menu_yl" href='./history_code.php?id=13&frequencytype=0' target="_blank"></a>
                                </div>
                            </div>
                            <div class="gct_r1">
                                <h3>江苏快三  第 <b><span class=nn id="lt_gethistorycode"><?=$rowc['issue']?></span></b> 期 <span id="lt_opentimebox" style="display:none;">&nbsp;&nbsp;<span id="waitopendesc">等待开奖</span>&nbsp;<span style="color:#F9CE46;" id="lt_opentimeleft" ></span></span><span id="lt_opentimebox2" style="display:none; color:#F9CE46;"><strong>&nbsp;&nbsp;正在开奖</strong></span></h3>
                                <div style="display:none;" class="tad" id="showadvbox"><a href="#"><img src='./images/company.gif' border="0" /></a></div>
                                <div class="gct_r_nubks" id="showcodebox">
                                    ...
                                    <div id="lt_trace_box" STYLE='display:none' class="trace_box">
                                        <div class="tabcon_n">
                                            ...
                                            <div class="unit1">
                                                ...
                                                <input type="hidden" name="lotteryid" id="lotteryid" value="13" />
...
<?php }}?>
        ...
        <script type="text/javascript">
            var pri_user_data = [
<?php
	$sqlb="select * from ssc_class where cid='13' order by id asc";

...
if ($pwd1 != $pwd2) {
	echo "<script language=javascript>alert('两次输入的密码不一致，请重新确认');window.location='getpass.php';</script>";
	exit;
}

if(strlen($pwd1)<6 || strlen($pwd1)>16 || preg_match('/(.+)\\1{2}/',$pwd1)){
	echo "<script>alert('密码不符合规则');window.location.href='getpass.php';</script>"; 
	exit;
}
...
if(empty($dduser)){
	...
	$pwd1= md5($pwd1);
	if($pwd1 == $pwd3){
		echo "<script language=javascript>alert('登陆密码不能和资金密码一样!');window.location='getpass.php';</script>";
		exit;	
	}
	if($pwd == $pwd1){
		echo "<script language=javascript>alert('登陆密码修改失败，新密码可能和原来密码一样');window.location='getpass.php';</script>";
		exit;
	}else{

...
$_SESSION["mainframe"] = '"./play_bjpk10.php"';
...
$sday=strtotime("2013-11-16");
$snum=394682;

$sqls="select * from ssc_nums where cid='15' and DATE_FORMAT(endtime, '%H:%i:%s')>='".date("H:i:s")."' order by id asc limit 1";
$rss=mysql_query($sqls) or  die("数据库修改出错1");
$nums=mysql_num_rows($rss);
$dymd=date("ymd");
$dymd2=date("Y-m-d");
if($nums==0){
	$sqls="select * from ssc_nums where cid='15' order by id asc limit 1";
...
$issue=sprintf("%06u",$snum+$sjc*179+intval($rows['nums'])-1);
...
if($flag=="gettime"){
	...
	$sqls="select * from ssc_bills where lotteryid='15' and username='".$_SESSION["username"]."' and adddate >='".$htime."' order by id desc limit 10";
	$rss=mysql_query($sqls) or  die("数据库修改出错1");
	$nums=mysql_num_rows($rss);
	echo '[';
	$i=1;
	...
	$sqla="select * from ssc_data where cid='15' and issue='".$_REQUEST['issue']."'";
	...
	if(empty($rowa)){
		...
		echo "{\"code\":[\"".$cn1."\",\"".$cn2."\",\"".$cn3."\",\"".$cn4."\",\"".$cn5."\",\"".$cn6."\",\"".$cn7."\",\"".$cn8."\",\"".$cn9."\",\"".$cn10."\"],\"issue\":\"".$_REQUEST['issue']."\",\"statuscode\":\"2\"}";//empty
	}
}else if($flag=="read"){
	...
	$sqlc="select * from ssc_data where cid='15' order by issue desc limit 1";
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script language="javascript" type="text/javascript" src="./js/lottery/min/face.15.js?modidate=20130415002"></script>
        <script language="javascript" type="text/javascript" src="./js/lottery/min/methods.15.js?modidate=20130415002"></script>
        <script language="javascript" type="text/javascript" src="./js/lottery/min/jquery.game.5.js?modidate=20130415002"></script>
                    ...
                    <div class="gm_con">
                        <div class="gm_con_tonew">
                            <div class="gct_l">
                                ...
                                <div class="gct_menu">
                                    <a class="gct_menu_yl" href='./history_code.php?id=15&frequencytype=0' target="_blank"></a>
                                </div>
                            </div>
                            <div class="gct_r3">
                                <h3>北京PK拾  第 <b><span class=nn id="lt_gethistorycode"><?=$rowc['issue']?></span></b> 期 <span id="lt_opentimebox" style="display:none;">&nbsp;&nbsp;<span id="waitopendesc">等待开奖</span>&nbsp;<span style="color:#F9CE46;" id="lt_opentimeleft" ></span></span><span id="lt_opentimebox2" style="display:none; color:#F9CE46;"><strong>&nbsp;&nbsp;正在开奖</strong></span></h3>
                                <div style="display:none;" class="tad" id="showadvbox"><a href="#"><img src='./images/company.gif' border="0" /></a></div>
                                <div class="gct_r_nub3" id="showcodebox">
...
<?php }}?>
        ...
        <script type="text/javascript">
            var pri_user_data = [
<?php
	$sqlb="select * from ssc_class where cid='15' order by id asc";
...
?>
            ...
            var pri_issues  = [
<?php
	$sqlb="select * from ssc_nums where cid='15' order by id asc";
	$rsb=mysql_query($sqlb) or  die("数据库修改出错!");
	$i=0;
	while ($rowb = mysql_fetch_array($rsb)){
		$number = sprintf("%06u",$snum+$sjc*179+$i); //期數
...
?>			];
            ...
            var pri_lotteryid = parseInt(15,10);
            var	pri_isdynamic = 1;
            var pri_showhistoryrecord = 1;
            var pri_ajaxurl   = './play_bjpk10.php';

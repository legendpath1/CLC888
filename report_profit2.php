...
if($username!=""){
	$s2=" username='".$username."' and regup='".$_SESSION["username"]."'";
}else{
...
$urls="starttime=".$starttime."&endtime=".$endtime."&username=".$username."&isrequery=".$isrequery;
...
<?php
			...
			while ($row = mysql_fetch_array($rsnewslist)){
				...
				$czm=$czm+$rowa['t1'];
				$txm=$txm+$rowa['t2']-$rowa['t3'];
				$tzm=$tzm+$rowa['t7']-$rowa['t13'];
				$fdm=$fdm+$rowa['t11']-$rowa['t15'];
				$zjm=$zjm+$rowa['t12']-$rowa['t16'];
				$tm=$tm+$ts;
?>
 ...
 ?>       
        ...
        <tr align="center">
            ...
            <td><b><?=number_format($czm,2)?></b></td>
            <td><b><?=number_format($txm,2)?></b></td>
            <td><b><?=number_format($tzm,2)?></b></td>
            <td><b><?=number_format($fdm,2)?></b></td>
            <td><b><?=number_format($zjm,2)?></b></td>
            <td><b><?=number_format($tm,2)?></b></td>
            <td></td>
        </tr>
<?php
		$sqla="select SUM(IF(types = 1, smoney, 0)) as t1,SUM(IF(types = 2, zmoney, 0)) as t2,SUM(IF(types = 3, smoney, 0)) as t3,SUM(IF(types = 7, zmoney, 0)) as t7,SUM(IF(types = 11, smoney, 0)) as t11,SUM(IF(types = 12, smoney, 0)) as t12,SUM(IF(types = 13, smoney, 0)) as t13,SUM(IF(types = 15, zmoney, 0)) as t15,SUM(IF(types = 16, zmoney, 0)) as t16,SUM(IF(types = 32, smoney, 0)) as t32,SUM(IF(types = 40, smoney, 0)) as t40 from ssc_record where (username='".$_SESSION["username"]."' or regfrom like '%&".$_SESSION["username"]."&%')".$s1."";
		$rsa = mysql_query($sqla);
		$rowa = mysql_fetch_array($rsa);
		$ts=$rowa['t11']-$rowa['t7']-$rowa['t9']+$rowa['t12']+$rowa['t13']-$rowa['t15']-$rowa['t16']; 
...
?>
    ...
    <tr class="page"><td class='b' colspan="9"><div style='text-align:right;'><ul class="pager">总计 <?=$total?> 条数据,  共 <?=$lastpg?> 页 , 当前第 <?=$page?> 页  |  <?php if($page>1){?><LI><A HREF="?<?=$urls?>&page=1">首页</A></LI><LI><A HREF="?<?=$urls?>&page=<?=$page-1?>">上页</A></LI><?php }?><?php for($i=$p1;$i<=$p2;$i++){

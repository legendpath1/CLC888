...
$_SESSION["mainframe"] = '"./play_fucai3d.php"';
...
$hstr="<table cellpadding=0 cellspacing=0 class=drawinfos><tr class=drawinfo><td width=180 class=drawinfo>期号</td><td width=212 class=drawinfo>开奖号码</td><td width=50><div id=hclose>关闭</div></td></tr>";
...
$sqls="select * from ssc_nums where cid='11' and DATE_FORMAT(endtime, '%H:%i:%s')>='".date("H:i:s")."' order by id asc limit 1";
$rss=mysql_query($sqls) or  die("数据库修改出错1");
$nums=mysql_num_rows($rss);
$dymd=date("y");
$dymd2=date("Y-m-d");
if($nums==0){
	$sqls="select * from ssc_nums where cid='11' order by id asc limit 1";
...
if($dymd2<$stopstart){
	$tnums=sprintf("%03d",date("z",strtotime($dymd2))+1);
}
if($dymd2>=$stopstart && $dymd2<=$stopend){
	$dymd=date("y",strtotime($stopend)+24*3600);
	$dymd2=date("Y-m-d",strtotime($stopend)+24*3600);
	$tnums=sprintf("%03d",date("z",strtotime($dymd2))+1-7);
}
if($dymd2>$stopstart){
	if(date("y",strtotime($stopstart))==date("y")){
		$tnums=sprintf("%03d",date("z",strtotime($dymd2))+1-7);
	}else{
		$tnums=sprintf("%03d",date("z",strtotime($dymd2))+1);
	}
...
if($flag=="gettime"){
	...
	$sqls="select * from ssc_bills where lotteryid='11' and username='".$_SESSION["username"]."' and adddate >='".$htime."' order by id desc limit 10";
	$rss=mysql_query($sqls) or  die("数据库修改出错1");
	$nums=mysql_num_rows($rss);
	echo '[';
	$i=1;
	...
	$sqla="select * from ssc_data where cid='11' and issue='".$_REQUEST['issue']."'";
	...
	if(empty($rowa)){
		...
		$cn1="k";
		$cn2="k";
		$cn3=substr($rowa['code'],0,1);
		$cn4=substr($rowa['code'],2,1);
		$cn5=substr($rowa['code'],4,1);
		echo "{\"code\":[\"".$cn1."\",\"".$cn2."\",\"".$cn3."\",\"".$cn4."\",\"".$cn5."\"],\"issue\":\"".$_REQUEST['issue']."\",\"statuscode\":\"2\",\"xt\":\"\",\"history\":\"".$hstr."\"}";//empty
	}
}else if($flag=="read"){
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script language="javascript" type="text/javascript" src="./js/lottery/min/face.11.js?modidate=20130415002"></script>
        <script language="javascript" type="text/javascript" src="./js/lottery/min/methods.11.js?modidate=20130415002"></script>
                ...
                <form>
                    ...
                    <div class="gm_con">
                        <div class="gm_con_tonew">
                            <div class="gct_l">
                                <h3>福彩3D</h3>
                                <p class="gct_now">正在销售 <strong>第 <span id="current_issue"><?=$issue?></span> 期</strong> </p>
                                <div class="clear"></div>
                                <div class="gct_time">
                                ...
                                <div class="clear"></div>
                                <div class="gct_menu">
                                    <a href='javascript:' id="hshow">开奖号码</a> | <a href='./history_coded.php?id=11&frequencytype=0' target="_blank">遗漏分析</a>
                                </div>
                            </div>
                            <div class="gct_r1" id="gctr">
                                <h3>福彩3D  第 <b><span class=nn id="lt_gethistorycode"><?=$rowc['issue']?></span></b> 期 <span id="lt_opentimebox" style="display:none;">&nbsp;&nbsp;<span id="waitopendesc">等待开奖</span>&nbsp;<span style="color:#F9CE46;" id="lt_opentimeleft" ></span></span><span id="lt_opentimebox2" style="display:none; color:#F9CE46;"><strong>&nbsp;&nbsp;正在开奖</strong></span></h3>
...
<?php }}?>
        ...
        <script type="text/javascript">
            var pri_user_data = [
<?php
	$sqlb="select * from ssc_class where cid='11' order by id asc";

...
if (strpos($_SESSION['flag'],'74') ){}else{ 
	echo "<script language=javascript>alert('对不起，您无此权限！');window.history.go(-1);</script>";
exit;}

$act=$_GET['act'];
if($act=="edit"){
	$sql="update ssc_set set zt = '".$_POST['zt']."', sign = '".$_POST['sign']."' where id ='".$_POST['sid']."'";
//	echo $sql;
	if (!mysql_query($sql)){
	  	die('Error: ' );
	}
  ...
  <br />
	<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
			<tr>
				...
				<td class="t_list_caption">热/新/无</td>
			...
			<tr class="t_list_tr_<?=$i%2?>" onMouseOver="this.style.backgroundColor='#E8F1FF'" onMouseOut="this.style.backgroundColor=''">
			    ...
			    关闭			    </td>
				<td align="center">
                <input name="sign" type="radio" value="2" <?php if($row['sign']==2){echo "checked='checked'";}?>/>
hot
			    &nbsp;&nbsp;
                <input type="radio" name="sign" value="1" <?php if($row['sign']==1){echo "checked='checked'";}?>/> 
                new
			    &nbsp;&nbsp;
                <input type="radio" name="sign" value="0" <?php if($row['sign']==0 || $row['sign']==""){echo "checked='checked'";}?>/>
不显示 </td>

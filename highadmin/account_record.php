...
if (strpos($_SESSION['flag'],'46') ){}else{ 
...
$urls="starttime=".$starttime."&endtime=".$endtime."&lotteryid=".$lotteryid."&methodid=".$methodid."&ordertype=".$ordertype."&mode=".$mode."&uname=".$uname."&username=".$username."&dan=".$dan;
...
$sql="select sum(smoney) as ssmoney,sum(zmoney) as szmoney from ssc_record where 1=1".$s1;
$rs = mysql_query($sql);
$row = mysql_fetch_array($rs);
$smoney=number_format($row['ssmoney'],2);
$zmoney=number_format($row['szmoney'],2);

$sql="select sum(smoney) as ssmoney,sum(zmoney) as szmoney from ssc_record where virtual=1".$s1;
$rs = mysql_query($sql);
$row = mysql_fetch_array($rs);
$vsmoney=number_format($row['ssmoney'],2);
$vzmoney=number_format($row['szmoney'],2);

$sql="select sum(smoney) as ssmoney,sum(zmoney) as szmoney from ssc_record where (virtual is null or virtual<>'1')".$s1;
$rs = mysql_query($sql);
$row = mysql_fetch_array($rs);
$ssmoney=number_format($row['ssmoney'],2);
$szmoney=number_format($row['szmoney'],2);
	...
	if($row['dan']==""){
      ...
      <tr>
		...
		if($i==$page){?> <span class='Font_R'><?=$i?></span> <?php }else{?> <A HREF="?<?=$urls?>&page=<?=$i?>"><?=$i?></A> <?php }}?><?php if($page!=$lastpg){?> <A HREF="?<?=$urls?>&page=<?=$page+1?>">下页</A> <A HREF="?<?=$urls?>&page=<?=$lastpg?>">尾页</A> <?php }?> | 转至 <SCRIPT LANGUAGE="JAVASCRIPT">function keepKeyNum(obj,evt){var  k=window.event?evt.keyCode:evt.which; if( k==13 ){ goPage(obj.value);return false; }} function goPage( iPage ){ if( !isNaN(parseInt(iPage)) ) { if(iPage< 0 ){iPage=0;} window.location.href="?<?=$urls?>&page="+iPage;}}</SCRIPT><INPUT onKeyPress="return keepKeyNum(this,event);" TYPE="TEXT" ID="iGotoPage" NAME="iGotoPage" size="3"> 页  &nbsp;<BUTTON onClick="javascript:goPage( document.getElementById('iGotoPage').value );return false;">GO</BUTTON></div></td>
              ...
              <tr>
                <td height="35" colspan="3">总收：<?=$smoney?>&nbsp;&nbsp;总支：<?=$zmoney?>&nbsp;&nbsp;实收：<?=$ssmoney?>&nbsp;&nbsp;实支：<?=$szmoney?>&nbsp;&nbsp;虚收：<?=$vsmoney?>&nbsp;&nbsp;虚支：<?=$vzmoney?></td>
              </tr>
            </table></td>
      </tr>
</table>

...
if (strpos($_SESSION['flag'],'23') ){}else{ 
...
if($username!=""){
	$s1=$s1." and ssc_bankcard.username='".$username."'";
}else{
	if($uname!=""){
		$s1=$s1." and ssc_bankcard.username='".$uname."'";
	}
}

if($realname!=""){
	$s1=$s1." and ssc_bankcard.realname='".$realname."'";
}

if($cardno!=""){
	$s1=$s1." and ssc_bankcard.cardno='".$cardno."'";
}

if($zt==1){
	$s1=$s1." and ssc_online.level<>''";
}

$urls="username=".$username."&realname=".$realname."&cardno=".$cardno."&uname=".$uname."&zt=".$zt;

$s1=$s1." order by id desc";
$sql="select ssc_bankcard.* from ssc_bankcard left join ssc_online on ssc_bankcard.username=ssc_online.username where 1=1".$s1;

$rs = mysql_query($sql);
$total = mysql_num_rows($rs);

$sql="select ssc_bankcard.*,ssc_online.level from ssc_bankcard left join ssc_online on ssc_bankcard.username=ssc_online.username where 1=1".$s1." limit $page2,$pagesize";
      ...
      <tr>
        <td class="top_list_td">          用户名:&nbsp;
          ...
          <select name="zt" id="zt">
            ...
            <option value="1" <?php if($zt==1){echo "SELECTED";}?>>在线</option>
          </select>
          &nbsp;

...
<div align="center"><br />
	<table width="95%" border="0" cellpadding="0" cellspacing="0" class="top_list">
      <tr>
        <td class="top_list_td icons_a5">　　您现在的位置是：报表管理 &gt; 团队统计报表</td>
      </tr>
    </table>
  	<br />
...
<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
			...
			<?php
				if($username!=""){
					$sqls="select SUM(cmoney) as tcmoney,SUM(tmoney) as ttmoney,SUM(gmoney) as tgmoney,SUM(fmoney) as tfmoney,SUM(wmoney) as twmoney,SUM(hmoney) as thmoney from ssc_membert where (username='".$username."' or regfrom like '%&".$username."&%')".$s1."";
				}else{
					$sqls="select SUM(cmoney) as tcmoney,SUM(tmoney) as ttmoney,SUM(gmoney) as tgmoney,SUM(fmoney) as tfmoney,SUM(wmoney) as twmoney,SUM(hmoney) as thmoney from ssc_membert where 1=1".$s1."";
				}
//				echo $sqla;
				$rss = mysql_query($sqls);
				$rows = mysql_fetch_array($rss);
				$tts=$rows['tgmoney']-$rows['thmoney']-$rows['tfmoney']-$rows['twmoney'];
				$tt1=$rows['tcmoney'];
				$tt32=$rows['thmoney'];
				$tt2=$rows['ttmoney'];
				$tt7=$rows['tgmoney'];
...
//				echo $sqla;
				...
				$ts=$rowa['tgmoney']-$rowa['thmoney']-$rowa['twmoney']-$rowa['tfmoney'];
			...
			<tr class="t_list_tr_<?=$i%2?>" onMouseOver="this.style.backgroundColor='#E8F1FF'" onMouseOut="this.style.backgroundColor=''">
	          ...
	          <td align="center"><?=number_format($rowa['thmoney'],2)?></td>
			  <td align="center"><?=number_format($rowa['tcmoney'],2)?></td>
			  <td align="center"><?=number_format($rowa['ttmoney'],2)?></td>
				<td align="center"><?=number_format($rowa['tgmoney'],2)?></td>
				<td align="center"><?=number_format($rowa['tfmoney'],2)?></td>
			    <td align="center"><?=number_format($rowa['twmoney'],2)?></td>
			    <td align="center">---</td>

...
if (strpos($_SESSION['flag'],'22') ){}else{ 
...
$FindTxt=$_REQUEST['FindTxt'];
$FindType=$_REQUEST['FindType'];
$zt=$_REQUEST['zt'];
...
if($_GET['act']=="add"){
	...
	if (preg_match("/[\x7f-\xff]/", $_POST['username'])) { 
	  echo "<script>alert('用户名含中文字符!');window.history.go(-1);</script>"; 
	  exit;
	}
...
//		}
		...
		if($SOPEN == 1)
		{
			$sapi_regResult = SAPI_Reg($_REQUEST['username'], $_REQUEST['userpass'], '', $_REQUEST['nickname']);
			if ($sapi_regResult[0] != 'SUCCESS')
			{
				echo "<script language=javascript>alert('" . $sapi_regResult[1] . "');window.history.go(-1);</script>";
				exit;
			}
		}
...
if($_GET['act']=="edit"){
	if($_POST['button']=="充 值" || $_POST['button']=="扣 款"){
		$sqla = "select cwpwd from ssc_config";
		$rsa = mysql_query($sqla);
		$rowa = mysql_fetch_array($rsa);
		$cwpwd=$rowa['cwpwd'];

		if($_POST['scode']!=$cwpwd){
			echo "<script>alert('请输入正确附加码！');window.location.href='mem_edit.php?act=edit&uid=".$_GET['uid']."';</script>"; 
			exit;
		}
		...
		if($_POST['button']=="充 值"){
			...
			$sqla="insert into ssc_record set dan='".$dan1."', uid='".$_GET['uid']."', username='".Get_mname($_GET['uid'])."', types='1', smoney=".$cmoney.",leftmoney=".$lmoney.", regtop='".$rowa['regtop']."', regup='".$rowa['regup']."', regfrom='".$rowa['regfrom']."', adddate='".date("Y-m-d H:i:s")."',virtual='" .$rowa['virtual']. "'";
		...
		if($_REQUEST['userpass']==""){
			if($_REQUEST['cwpassword']==""){
				...
				if($SOPEN == 1)
				{
					$arr = SAPI_ChangePassword2($_REQUEST["username_hide"],'',md5($_REQUEST['cwpassword']));
				...
				$sql = "update ssc_member set cwpwd='" . md5($_REQUEST['cwpassword']) . "', nickname='" . $_REQUEST['nickname'] . "',  flevel='" . $flevel . "', zc='" . $zc . "', pe='" . $_REQUEST['pe'] . "', banks='" . $_REQUEST['banks'] . "', virtual='" . $_REQUEST['virtual'] . "', level='" . $_REQUEST['usertype'] . "' where id='" . $_GET['uid'] . "'";
			}
		}else{
			...
			if($SOPEN == 1)
			{
				$arr = SAPI_ChangePassword($_REQUEST["username_hide"],$_REQUEST['password_hide'],md5($_REQUEST['userpass']));
				if($arr['state'] != 'SUCCESS')
				{
					echo "<script language='javascript'>alert('" . $arr['tips'] . "');history.go(-1);</script>";
					exit;
			...
			if($_REQUEST['cwpassword']==""){
				$sql = "update ssc_member set password='" . md5($_REQUEST['userpass']) . "', nickname='" . $_REQUEST['nickname'] . "',  flevel='" . $flevel . "', zc='" . $zc . "', pe='" . $_REQUEST['pe'] . "', banks='" . $_REQUEST['banks'] . "', virtual='" . $_REQUEST['virtual'] . "', level='" . $_REQUEST['usertype'] . "' where id='" . $_GET['uid'] . "'";		
			}else{
				...
				$sql = "update ssc_member set password='" . md5($_REQUEST['userpass']) . "', cwpwd='" . md5($_REQUEST['cwpassword']) . "', nickname='" . $_REQUEST['nickname'] . "', flevel='" . $flevel . "', zc='" . $zc . "', pe='" . $_REQUEST['pe'] . "', banks='" . $_REQUEST['banks'] . "', virtual='" . $_REQUEST['virtual'] . "', level='" . $_REQUEST['usertype'] . "' where id='" . $_GET['uid'] . "'";
			}
		}
		$exe = mysql_query($sql) or  die("数据库修改出错2");
...
if($_GET['act']=="vipzt"){
	mysql_query("update ssc_member set vip='".$_GET['vip']."' where id=".$_GET['uid']);
	
	amend("修改用户 ".Get_mname($_GET['uid'])."vip状态为".$_GET['vip']);	
	
	echo "<script>alert('修改用户vip状态成功！');window.location.href='mem_list.php';</script>"; 
	exit;	
}
...
$urls="FindType=".$FindType."&FindTxt=".$FindTxt."&UP_ID=".$UP_ID."&Compositor=".$Compositor."&Ascending=".$Ascending."&userid=".$userid;
    ...
    <table width="95%" border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td class="top_list_td">排序:
          ...
          &nbsp;序号:
          <input type="text" name="userid" maxlength="30" size="10" value="" class="inpa">
          &nbsp;
	...
	<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
        <tr>
          ...
          <td class="t_list_caption">开户时间</td>
            ...
            <td class="t_list_caption">vip</td>
            <td class="t_list_caption">银行卡</td>
            <td class="t_list_caption">套餐</td>
		...
		?>
            <tr class="t_list_tr_0" onMouseOver="this.style.backgroundColor='#FFFFA2'" onMouseOut="this.style.backgroundColor=''">
                ...
                <td><?php if($row['vip']==1){echo "<a href=?act=vipzt&uid=".$row['id']."&vip=0><span class='Font_R'>vip</span></a>";}else{echo "<a href=?act=vipzt&uid=".$row['id']."&vip=1><span class='Font_G'>---</span></a>";}?></td>

...
//			}
			...
			if(strpos($row['codes'],$n1.$n2.$n3)===false && strpos($row['codes'],$n1.$n3.$n2)===false && strpos($row['codes'],$n2.$n1.$n3)===false && strpos($row['codes'],$n2.$n3.$n1)===false && strpos($row['codes'],$n3.$n1.$n2)===false && strpos($row['codes'],$n3.$n2.$n1)===false){
				...
				if($n1==$n2 || $n1==$n3 || $n2==$n3){//组三
					mysql_query("update ssc_bills set zt=".$signa.",prize=".$rates[0]."*times where id='".$row['id']."'");										
				}else{				
					mysql_query("update ssc_bills set zt=".$signa.",prize=".$rates[1]."*times where id='".$row['id']."'");						
				}
			}
...
//			echo $numa.$row['codes'].$numb."<br>";
			...
			if(strpos($row['codes'],$numstr)===false){
				...
				mysql_query("update ssc_bills set zt=".$signa.",prize=".$rates[$numa]."*times where id='".$row['id']."'");
			}
...
//			echo $nums;
				mysql_query("update ssc_bills set zt=".$signa.",prize=".$rates[$nums-3]."*times where id='".$row['id']."'");
			}
//			echo $nb[2];
				...
				if($rowb['regfrom']!=""){
					...
					for ($ia=0; $ia<count($regfrom); $ia++) {
						...
						if(($sstrb-$sstrp)>0){
							...
							$rowa = mysql_fetch_array($rsa);
							$dan1 = sprintf("%07s",strtoupper(base_convert($rowa['id']+1,10,36))).sprintf("%02s",strtoupper(base_convert(mt_rand(0,1295),10,36)));
							$sqla="insert into ssc_record set lotteryid='".$rowb['lotteryid']."', lottery='".$rowb['lottery']."', dan='".$dan1."', dan1='".$dan2."', dan2='".$rowb['dan']."', uid='".Get_memid($susername)."', username='".$susername."', issue='".$rowb['issue']."', types='11', mid='".$rowb['mid']."', mode='".$rowb['mode']."', smoney=".($rowb['money']*($sstrb-$sstrp)/100).",leftmoney=".(Get_mmoney($susername)+$rowb['money']*($sstrb-$sstrp)/100).", cont='".$rowb['cont']."', regtop='".$rowb['regtop']."', regup='".$rowb['regup']."', regfrom='".$rowb['regfrom']."', adddate='".date("Y-m-d H:i:s")."'";
							$exe=mysql_query($sqla) or  die("数据库修改出错4!!!");
			
							$sqla="update ssc_member set leftmoney=leftmoney+".($rowb['money']*($sstrb-$sstrp)/100)." where username='".$susername."'"; 
							$exe=mysql_query($sqla) or  die("数据库修改出错12!!!");
						}
					}
				}

...
if($_GET['act']=="tx"){
	if($_POST['button']=="审核"){
		$sqla="update ssc_drawlist set cno='".$_POST['dan']."', cdate='".date("Y-m-d H:i:s")."',tag='".$_POST['cont']."', zt='1' where id='".$_POST['sid']."'";
		$exe=mysql_query($sqla) or  die("数据库修改出错6!!!");

		$sqla="insert into ssc_message set username='".$_POST["uid"]."',types='充提消息',topic='&nbsp;&nbsp;&nbsp;&nbsp;恭喜您，提现【<font color=blue>".$_POST['did']."</font>】已成功处理', content='<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您的提现申请【编号：<font color=blue>".$_POST['did']."</font>】已经成功处理, 请注意查看您的帐变信息，如果有任何疑问请联系我们在线客服。</p>',adddate='".date("Y-m-d H:i:s")."'";
//		echo $sqla;
		$exe=mysql_query($sqla) or  die("数据库修改出错6!!!");
		
	}else if($_POST['button']=="拒绝"){
		$sqla="update ssc_drawlist set cno='".$_POST['dan']."', cdate='".date("Y-m-d H:i:s")."',tag='".$_POST['cont']."', zt='2' where id='".$_POST['sid']."'";	
		$exe=mysql_query($sqla) or  die("数据库修改出错6!!!");

		$sqlb = "select * from ssc_drawlist WHERE id='" . $_POST["sid"] . "'";
		...
		$sqla="insert into ssc_record set dan='".$dan1."', uid='".$rowa['id']."', username='".$rowa['username']."', types='3', smoney=".$rowb['money'].",leftmoney=".$lmoney.", regtop='".$rowa['regtop']."', regup='".$rowa['regup']."', regfrom='".$rowa['regfrom']."', adddate='".date("Y-m-d H:i:s")."',virtual='" .$rowa['virtual']. "'";
		$exe=mysql_query($sqla) or  die("数据库修改出错6!!!");
...
<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
  ...
  <?php
		...
		{
$sqla="select vip from ssc_member where id='".$row['uid']."'";
$rsa = mysql_query($sqla);
$rowa = mysql_fetch_array($rsa);
if($rowa['vip']==1){$vip="vip";}else{$vip="";}
  ...
  <tr class="t_list_tr_0" onMouseOver="this.style.backgroundColor='#FFFFA2'" onMouseOut="this.style.backgroundColor=''">
    ...
    <td><?=$row['dan']?>
      ...
      <input type="hidden" name="did" value="<?=$row['dan']?>" />
    ...
    <td id="status"><a href="account_txs.php?sid=<?=$row['id']?>">处理</a></td>
    <td>
      <input name="button" type="submit" class="but1" id="button" value="拒绝"  onClick="return confirm('确认要拒绝吗?');"/>    </td>
  </tr>
  </form>
  <?php
		}

...
<select name="zt" id="zt">
  <option value="0" <?php if($zt==0){echo "SELECTED";}?>>充值成功</option>
</select>
...
<table width="95%" border="1" cellpadding="0" cellspacing="0" bordercolor="#DDDDDD" class="t_list">
  ...
  <tr class="t_list_tr_0" onMouseOver="this.style.backgroundColor='#FFFFA2'" onMouseOut="this.style.backgroundColor=''">
    ...
    <td><?=$row['tag']?></td>
    <td id="status"><?php if($row['zt']=="0"){?><font color="#FF0000">正在充值</font><?php }else if($row['zt']=="1"){?><font color=#669900>充值成功</font><?php }?></td>
  ...
  <tr>
    <td colspan="12" class="t_list_bottom"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td><div style='text-align:right;'>充值总额:<?=$srmoney?>上分总额:<?=$smoney?> 总计  <?=$total?> 条数据,  共 <?=$lastpg?> 页 , 当前第 <?=$page?> 页  |  <?php if($page>1){?> <A HREF="?<?=$urls?>&page=1">首页</A> <A HREF="?<?=$urls?>&page=<?=$page-1?>">上页</A><?php }?><?php for($i=$p1;$i<=$p2;$i++){

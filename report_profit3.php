...
<?php
			...
			while ($row = mysql_fetch_array($rsnewslist)){
				...
				$czm=$czm+$rowa['tcmoney'];
				$txm=$txm+$rowa['ttmoney'];
				$tzm=$tzm+$rowa['tgmoney'];//投注
				$fdm=$fdm+$rowa['tfmoney'];//返点
				$zjm=$zjm+$rowa['twmoney'];//中奖
				$hdm=$hdm+$rowa['thmoney'];//活动
				$tm=$tm+$ts;
?>
 ...
 ?>
        ...
        <tr align="center">
            ...
            <td><b><?=number_format($hdm,2)?></b></td>
            <td><b><?=number_format($tm,2)?></b></td>
            <td></td>
        </tr>
<?php
		$sqla="select SUM(cmoney) as tcmoney,SUM(tmoney) as ttmoney,SUM(gmoney) as tgmoney,SUM(fmoney) as tfmoney,SUM(wmoney) as twmoney,SUM(hmoney) as thmoney from ssc_membert where (username='".$_SESSION["username"]."' or regfrom like '%&".$_SESSION["username"]."&%')".$s1."";
		$rsa = mysql_query($sqla);
		$rowa = mysql_fetch_array($rsa);
				$ts=$rowa['twmoney']+$rowa['thmoney']+$rowa['tfmoney']-$rowa['tgmoney']; 

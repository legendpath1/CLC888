...
$_SESSION["mainframe"] = '"./play_cq115.php"';
...
$hstr="<table cellpadding=0 cellspacing=0 class=drawinfos><tr class=drawinfo><td width=180 class=drawinfo>期号</td><td width=202 class=drawinfo>开奖号码</td><td width=60><a href='./history_coded.php?id=10&frequencytype=0' target=_blank>更多号码</a></td></tr>";
...
$sqls="select * from ssc_nums where cid='10' and DATE_FORMAT(endtime, '%H:%i:%s')>='".date("H:i:s")."' order by id asc limit 1";
...
if($flag=="gettime"){
	...
	$sqls="select * from ssc_bills where lotteryid='10' and username='".$_SESSION["username"]."' and adddate >='".$htime."' order by id desc limit 10";
	$rss=mysql_query($sqls) or  die("数据库修改出错1");
	$nums=mysql_num_rows($rss);
	echo '[';
	$i=1;
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script language="javascript" type="text/javascript" src="./js/lottery/min/face.10.js?modidate=20130415002"></script>
        <script language="javascript" type="text/javascript" src="./js/lottery/min/methods.10.js?modidate=20130415002"></script>
        <script language="javascript" type="text/javascript" src="./js/lottery/min/jquery.game.2.js?modidate=20130415002"></script>
                ...
                <form>
                    ...
                    <div class="gm_con">
                        <div class="gm_con_tonew">
                            <div class="gct_l">
                                <h3>重庆十一选五</h3>
                                <p class="gct_now">正在销售 <strong>第 <span id="current_issue"><?=$issue?></span> 期</strong> 总共: <strong><span id="current_sale">85</span></strong> 期</span></p>
                                <div class="clear"></div>
                                <div class="gct_time">
                                ...
                                <div class="clear"></div>
                                <div class="gct_menu">
                                    <a href='./history_coded.php?id=10&frequencytype=0' target="_blank">胆码走势</a> | <a href='javascript:' id="hshow">开奖号码</a> | <a href='./history_coded.php?id=10&frequencytype=0' target="_blank">遗漏分析</a>
                                </div>
                            </div>
                            <div class="gct_r2" id="gctr">
                                <h3>重庆十一选五  第 <b><span class=nn id="lt_gethistorycode"><?=$rowc['issue']?></span></b> 期 <span id="lt_opentimebox" style="display:none;">&nbsp;&nbsp;<span id="waitopendesc">等待开奖</span>&nbsp;<span style="color:#F9CE46;" id="lt_opentimeleft" ></span></span><span id="lt_opentimebox2" style="display:none; color:#F9CE46;"><strong>&nbsp;&nbsp;正在开奖</strong></span></h3>
                                    ...
                                    <div id="lt_trace_box" STYLE='display:none' class="trace_box">
                                        <div class="tabcon_n">
                                            ...
                                            <div class="unit1">
                                                ...
                                                <input type="hidden" name="lotteryid" id="lotteryid" value="10" />
...
<?php }}?>
        ...
        <script type="text/javascript">
            var pri_user_data = [
<?php
	$sqlb="select * from ssc_class where cid='10' order by id asc";
...
?>
            ...
            var pri_issues  = [<?php
	$sqlb="select * from ssc_nums where cid='10' order by id asc";
	$rsb=mysql_query($sqlb) or  die("数据库修改出错!");
	while ($rowb = mysql_fetch_array($rsb)){
...
?>];
            ...
            var	pri_issuecount= 85;
            var pri_servertime= '<?=date("Y-m-d H:i:s")?>';
            var pri_lotteryid = parseInt(10,10);
            var	pri_isdynamic = 1;
            var pri_showhistoryrecord = 1;
            var pri_ajaxurl   = './play_cq115.php';
